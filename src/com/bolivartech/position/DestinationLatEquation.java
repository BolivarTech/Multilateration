 ...
 * @author jbolivarg
 */
public class DestinationLatEquation extends NAFunction {
    private double lat1; 
    private double lon1;
    private double Distance,Azimut;

    public DestinationLatEquation(double lon1, double lat1, double Distance, double Azimut) {
        this.lat1 = lat1;
        this.lon1 = lon1;
    ...
    public double Eval(double[] x) throws UtilsException {
        ...
        return Math.asin(Math.sin(lat1) * Math.cos(centralAngle) + Math.cos(lat1) * Math.sin(centralAngle) * Math.cos(Azimut))-lat2;
    }
